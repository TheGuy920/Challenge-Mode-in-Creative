    ...
    self.sv = {
        saved = {
            world = sm.world.createWorld(
                "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/Core/World.lua",
                "World"
            )
        }
    }
...
function Game.client_initializePackMenu(self, force)
    ...
    if not sm.isHost then
        ...
        if sm.exists(self.MenuInstance.pack.gui) and force ~= true then
            ...
            self.MenuInstance.pack.gui =
                sm.gui.createGuiFromLayout(
                "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Gui/Layouts/ChallengeModeMenuPack.layout"
            )
            self.MenuInstance.pack.gui:setVisible("RecordContainer", false)
            self.MenuInstance.pack.gui_table =
                sm.json.open(
                "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/CustomGame/Json/ChallengeModeMenuPack.json"
            )
...
function Game.server_exitToMenu2(self, data)
    ...
    if data.first == true then
        ...
        if not sm.exists(self.sv.saved.world) then
            self.sv.saved.world =
                sm.world.createWorld("$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/Core/World.lua", "World")
        end
        self.respawn_all = 2
        self:server_updateGameState(States.PackMenu)
        self.ready = true
...
function Game.client_tmpGui( self, gui )
    self.MenuInstance.waiting.gui = sm.gui.createGuiFromLayout("$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Gui/Layouts/waiting.layout")
...
function Game.client_initializeBuildMenu(self, force)
    ...
    if sm.exists(self.MenuInstance.build.gui) and force ~= true then
        ...
        self.MenuInstance.build.gui =
            sm.gui.createGuiFromLayout(
            "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Gui/Layouts/ChallengeBuilder.layout"
        )
        self.MenuInstance.build.gui:setVisible("RecordContainer", false)
        self.MenuInstance.build.level_table =
            sm.json.open("$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/CustomGame/Json/ChallengeBuilder.json")
...
function Game.client_initializePlayMenu(self, uuid, force)
    ...
    if sm.exists(self.MenuInstance.play.gui) and force ~= true then
        ...
        self.MenuInstance.play.gui =
            sm.gui.createGuiFromLayout(
            "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Gui/Layouts/ChallengeModeMenuPlay.layout"
        )
        self.MenuInstance.play.gui:setVisible("RecordContainer", false)
        self.MenuInstance.play.play_table =
            sm.json.open(
            "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/CustomGame/Json/ChallengeModeMenuPlay.json"
        )
...
function Game.server_saveAsOverride(self, _table)
    local path = _table.path
    local data = _table.data
    local header = "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Overrides/"
...
function Game.client_onUpdate(self, deltaTime)
    ...
    if self.MenuInstance.blur.blur_gui:isActive() and sm.exists(self.MenuInstance.blur.gui) and self.MenuInstance.blur.gui:isActive() then
    ...
    if self.inverse_camera_interpolate and sm.isHost then
        ...
        if posa == posb then
            ...
            self.MenuInstance.blur.gui =
                sm.gui.createGuiFromLayout(
                "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Gui/Layouts/__invis.layout",
                false,
                {
                    isHud = true,
                    isInteractive = false,
