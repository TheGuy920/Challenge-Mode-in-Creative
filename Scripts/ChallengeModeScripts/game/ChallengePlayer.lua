...
function ChallengePlayer.client_onCancel( self )
	...
	_G.sm.globals.g_effectManager:cl_cancelAllCinematics()
end
...
function ChallengePlayer.client_onCreate( self )
	...
	if self.player == sm.localPlayer.getPlayer() then
		if _G.sm.globals.g_survivalHud and sm.exists(_G.sm.globals.g_survivalHud) then
			_G.sm.globals.g_survivalHud:open()
		end
	end
end
...
function ChallengePlayer.client_onClientDataUpdate( self, data )
	...
	if sm.localPlayer.getPlayer() == self.player then
		...
		if _G.sm.globals.g_survivalHud and sm.exists(_G.sm.globals.g_survivalHud) then
			_G.sm.globals.g_survivalHud:setVisible( "HealthBar", data.enableHealth )
			_G.sm.globals.g_survivalHud:setSliderData( "Health", data.stats.maxhp * 10 + 1, data.stats.hp * 10 )
		
			if not _G.sm.globals.g_survivalHud:isActive() then
				_G.sm.globals.g_survivalHud:open()
			end
		end
	...
	if params.container == sm.localPlayer.getInventory() then
		for i, item in ipairs( params.changes ) do
			if item.difference > 0 then
				_G.sm.globals.g_survivalHud:addToPickupDisplay( item.uuid, item.difference )
			end
		end
	end
end
...
function ChallengePlayer.server_onFixedUpdate( self, dt )
	...
	local character = self.player:getCharacter()
	if character and self.sv.saved.isConscious --[[and not _G.sm.globals.g_godMode]] then
