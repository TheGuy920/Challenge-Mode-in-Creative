...
ChallengeVictoryWorld.terrainScript = "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/challenge/terrain_challenge.lua"
...
function ChallengeVictoryWorld.client_onCreate( self )
	...
	self.cl.approvedGui = sm.gui.createGuiFromLayout( "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Gui/Layouts/Banner_Approved.layout", false, { isHud = true, isInteractive = false, needsCursor = false, hidesHotbar = true } )
	self.cl.approvedTimer = Timer()
	self.cl.approvedTimer:start( ApprovedTickTime )

	self.cl.stuntmanGui = sm.gui.createGuiFromLayout( "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Gui/Layouts/Stuntman_Unlocked.layout", false, { backgroundAlpha = 0.6 } )
	...
	self.cl.closedStuntmanGui = not _G.sm.globals.g_showMasterMechanicTrialsReward

	self.cl.finishGui = sm.gui.createGuiFromLayout( "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Gui/Layouts/Victory_Finish.layout" )
...
function ChallengeVictoryWorld.client_onUpdate( self, dt )
	...
	local cutscene = _G.sm.globals.g_effectManager:cl_getWorldNamedEffect( self, "cinematic.challengemodecomplete" )
	if cutscene then
...
function ChallengeVictoryWorld.cl_n_spawned( self )
	if self.cl.watchedCutscene then
		...
		_G.sm.globals.g_effectManager:cl_playNamedCinematic( "cinematic.challengemodecomplete", callbacks )
	end
end
