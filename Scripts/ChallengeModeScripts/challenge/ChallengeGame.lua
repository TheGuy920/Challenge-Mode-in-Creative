dofile("$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/challenge/game_util.lua")
dofile("$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/challenge/world_util.lua")
dofile("$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/game/challenge_shapes.lua")
...
function InitializeChallengeGame()
	...
	function ChallengeGame.server_onCreate(self)
		...
		_G.sm.globals.g_disableScrapHarvest = true

		_G.sm.globals.g_unitManager = UnitManager()
		_G.sm.globals.g_unitManager:sv_onCreate()
		...
		if self.data.startLevelIndex ~= nil then
			...
			for _, level in ipairs(self.play.levelList) do
				...
				level.data.tiles[#level.data.tiles + 1] =
					"$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Terrain/Tiles/challengemode_env_DT.tile"
			end
		else
			-- Build mode
			self.build = {}
			...
			self.build.level.data.tiles[#self.build.level.data.tiles + 1] =
				"$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Terrain/Tiles/challengebuilder_env_DT.tile"

			sm.game.setLimitedInventory(false)
			_G.sm.globals.g_inventoriesBuildMode = {}
	...
	function ChallengeGame.server_onFixedUpdate(self)
		...
		_G.sm.globals.g_unitManager:sv_onFixedUpdate()
	end
	...
	function ChallengeGame.server_startTest(self, levelData)
		...
		for i, player in ipairs(sm.player.getAllPlayers()) do
			...
			_G.sm.globals.g_inventoriesBuildMode[player.id] = inventoryList
		end
	...
	function ChallengeGame.server_stopTest(self)
		...
		sm.event.sendToWorld(self.activeWorld, "server_spawnCharacter", {players = sm.player.getAllPlayers(), inventory=_G.sm.globals.g_inventoriesBuildMode})
	end

	function ChallengeGame.server_start(self)
		_G.sm.globals.g_savedCreations = {}
		_G.sm.globals.g_inventoriesPlayMode = {}
		_G.sm.globals.g_savedChests = {}
		_G.sm.globals.g_tutorial = false
		_G.sm.globals.g_droppedItems = {}
	...
	function ChallengeGame.server_onPlayerJoined(self, player)
		...
		_G.sm.globals.g_unitManager:sv_onPlayerJoined(player)
	end
	...
	function ChallengeGame.sv_loadVictoryLevel(self)
		...
		worldData.tiles[#worldData.tiles + 1] =
			"$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Terrain/Challangemode_victoryscene.tile"
		self.world =
			sm.world.createWorld(
			"$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/challenge/ChallengeVictoryWorld.lua",
	...
	function ChallengeGame.server_loadLevel(self, loadJsonData, loadSaveData)
		...
		if self.build and not self.build.testing then
			worldScriptFilename =
				"$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/challenge/BuilderWorld.lua"
			worldScriptClass = "BuilderWorld"
		else
			worldScriptFilename =
				"$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/challenge/ChallengeWorld.lua"
	...
	function ChallengeGame.server_loadJsonData(self)
		...
		if levelData.startCreations then
			for _, creation in ipairs(levelData.startCreations) do
				...
				_G.sm.globals.g_savedCreations[#g_savedCreations + 1] = blueprintJson
			end
		end
		...
		if levelData.chestContents then
			_G.sm.globals.g_savedChests[1] = {}
			_G.sm.globals.g_savedChests[1] = levelData.chestContents
		end

		_G.sm.globals.g_tutorial = levelData.tutorial
		_G.sm.globals.g_droppedItems = {}
	end
	...
	function ChallengeGame.server_loadSaveData(self)
		...
		if saveData and saveData.creations then
			_G.sm.globals.g_savedCreations = saveData.creations
			...
			for shape in pairs(availableShapes) do
				if availableShapes[shape] > 0 then
					_G.sm.globals.g_savedChests[1][index] = {uuid = shape, quantity = availableShapes[shape]}
					index = index + 1
				end
			end
		end
	...
	function ChallengeGame.server_onChallengeCompleted(self, victoryParams)
		...
		if self.play and victoryParams.canSaveCreation == true then
			saveData = {creations = _G.sm.globals.g_savedCreations}
		end
	...
	function ChallengeGame.client_onCreate(self)
		...
		_G.sm.globals.g_effectManager = EffectManager()
		_G.sm.globals.g_effectManager:cl_onCreate()
		...
		if _G.sm.globals.g_unitManager == nil then
			assert(not sm.isHost)
			_G.sm.globals.g_unitManager = UnitManager()
		end
		_G.sm.globals.g_unitManager:cl_onCreate()

		-- Survival HUD, for HP (TODO: challengeHUD)
		if _G.sm.globals.g_survivalHud == nil or not sm.exists(_G.sm.globals.g_survivalHud) then
			_G.sm.globals.g_survivalHud = sm.gui.createSurvivalHudGui()
		end
		--assert(g_survivalHud)
		_G.sm.globals.g_survivalHud:setVisible("FoodBar", false)
		_G.sm.globals.g_survivalHud:setVisible("WaterBar", false)
	end
	...
	function ChallengeGame.client_onLoadingScreenLifted(self)
		_G.sm.globals.g_effectManager:cl_onLoadingScreenLifted()
	end
	...
	function ChallengeGame.client_onChallengeStarted(self, params)
		...
		if sm.isHost then
			_G.sm.globals.g_showMasterMechanicTrialsReward =
