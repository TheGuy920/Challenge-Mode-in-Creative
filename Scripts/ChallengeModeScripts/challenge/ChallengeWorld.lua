dofile( "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/challenge/ChallengeBaseWorld.lua")
...
dofile( "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/game/challenge_tools.lua" )
dofile( "$SURVIVAL_DATA/Scripts/game/managers/WaterManager.lua" )

ChallengeWorld = class( ChallengeBaseWorld )
ChallengeWorld.terrainScript = "$CONTENT_ee7f6b44-e9e8-4636-89ce-e7f5fd41c070/Scripts/ChallengeModeScripts/challenge/terrain_challenge.lua"
...
function ChallengeWorld.server_onFixedUpdate( self )
	...
	for idx, droppedItem in Reverse_ipairs( _G.sm.globals.g_droppedItems ) do
		if currentTick > droppedItem.tick and not sm.exists( droppedItem.hvs ) then
			table.remove( _G.sm.globals.g_droppedItems, idx )
		end
	end
...
function ChallengeWorld.server_onCellCreatedFinish( self, x, y )
	...
	if _G.sm.globals.g_tutorial then
		self.tutorialIntroTimer = 0
		self.tutorialStage = "Intro"
	end

	-- Recreate dropped items
	if _G.sm.globals.g_droppedItems ~= nil then
		for _, droppedItem in ipairs( _G.sm.globals.g_droppedItems ) do
...
function ChallengeWorld.server_loadSavedInventory( self, player )
	...
	if _G.sm.globals.g_inventoriesPlayMode[player.id] == nil then
		local inventoryList = {}
		...
		_G.sm.globals.g_inventoriesPlayMode[player.id] = inventoryList
	end

	-- Add to inventory
	local savedInventory = _G.sm.globals.g_inventoriesPlayMode[player.id]
...
function ChallengeWorld.server_loadWorldContent( self, data )
	...
	--Creations
	if _G.sm.globals.g_savedCreations ~= nil then -- _G.sm.globals.g_savedCreations can be nil if the challenge is started with the ChallengeStarter and nothing has been built
		for _, creation in ipairs( _G.sm.globals.g_savedCreations ) do
	...
	--Chests
	sm.container.beginTransaction()
	for index, savedChest in ipairs( _G.sm.globals.g_savedChests ) do
...
function ChallengeWorld.server_saveWorldContent( self, playerCreationBodies )
	...
	if self.chests then
		...
		for index, chest in ipairs(self.chests) do
			_G.sm.globals.g_savedChests[index] = {}
			if sm.exists( chest ) then
				...
				for slot = 1, container:getSize() do
					...
					_G.sm.globals.g_savedChests[index][slot] = { uuid = tostring( item.uuid ), quantity = item.quantity }
				end
			end
		end
	end
...
function ChallengeWorld.server_onProjectile( self, hitPos, hitTime, hitVelocity, _, attacker, damage, userData, hitNormal, target, projectileUuid )
	...
	if userData and userData.lootUid then
		...
		table.insert( _G.sm.globals.g_droppedItems, droppedItem )
...
function ChallengeWorld.client_onUpdate( self, dt )
	if self.waitingForDeath == true then return end
	if _G.sm.globals.g_effectManager ~= nil then
		_G.sm.globals.g_effectManager:cl_onWorldUpdate( self )
	end
...
function ChallengeWorld.client_onCellLoaded( self, x, y )
	...
	if _G.sm.globals.g_effectManager ~= nil then
	_G.sm.globals.g_effectManager:cl_onWorldCellLoaded( self, x, y )
	end
end
...
function ChallengeWorld.client_onCellUnloaded( self, x, y )
	...
	if _G.sm.globals.g_effectManager ~= nil then
	_G.sm.globals.g_effectManager:cl_onWorldCellUnloaded( self, x, y )
	end
end
...
function ChallengeWorld.client_spawned( self, params )
	...
	if params.player == sm.localPlayer.getPlayer() then
		...
		if _G.sm.globals.g_effectManager and params.playCutscene then
			_G.sm.globals.g_effectManager:cl_playNamedCinematic( "LevelIntro" )
		end
	end
end
